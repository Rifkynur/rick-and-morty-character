...
const EpisodeCard = ({ air_date, episode, id, name }: EpisodeCardProps) => {
  ...
  return (
    <div
      className="shadow-lg rounded-lg p-6 flex flex-col gap-3 items-center justify-center cursor-pointer transition-all duration-300 hover:scale-105"
      onClick={() => router.push(`/episode/${id}`)}
    >
      <h2 className="font-bold text-lg text-center font-rickAndMorty">
        {name}
      </h2>
      <h3 className="text-black/70">{air_date}</h3>
